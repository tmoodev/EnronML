...
features_fin = ['salary', 'deferral_payments', 'total_payments', 
'loan_advances', 'bonus', 'restricted_stock_deferred', 'deferred_income', 
'total_stock_value', 'expenses', 'exercised_stock_options', 'other', 
'long_term_incentive', 'restricted_stock', 'director_fees'] 

features_em = ['to_messages', 'from_poi_to_this_person', 'from_messages', 
'from_this_person_to_poi', 'shared_receipt_with_poi']

poi_flag = ['poi']

features_list = poi_flag + features_em + features_fin
...
count_data_points = len(data_dict)
print("Total number of data points: %i" %len(data_dict))

# Flagged poi vs non-poi
poi = 0
for person in data_dict:
    if data_dict[person]['poi'] == True:
       poi += 1
print("Number of pois: %i" % poi)
print("Number of non-pois: %i" % (len(data_dict) - poi))
       
# Number of features used
all_features = data_dict[data_dict.keys()[0]].keys()
print("There are %i features per person in the dataset, and %i features \
...
print("Ideal features:")
scores = zip(new_features_list[1:],selector.scores_)
sorted_scores = sorted(scores, key = lambda x: x[1], reverse=True)
print sorted_scores
optimized_features_list = poi_flag + list(map(lambda x: x[0], sorted_scores))[0:k]
...
print("Naive Bayes (New Features)")
...
print("Logistic Regression Model (Original Features)")
evaluate_clf(lo_grid_search, features, labels, lo_param)

classifier__tol = 1, 
classifier__C = 0.1, 
print("Logistic Regression Model (New features)")
